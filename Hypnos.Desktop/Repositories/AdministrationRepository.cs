...
namespace Wholesale.Desktop.Repositories
{
    public class AdministrationRepository : RepositoryBase
    {
        ...
        public int AddUser(string userJson) => ExecuteCommandAuth("AddUser", new SqlParameter("@user_json", userJson));

        public int EditUser(string userJson) => ExecuteCommandAuth("EditUser", new SqlParameter("@user_json", userJson));
    }
}
